...
async function listCategories(){ 
  ...
  el.innerHTML=`<h2>管理類型</h2><div class="muted">（保留原本類型管理掛載點）</div>`; 
...
async function listBudgets(){ 
  const el=document.createElement('div'); 
  el.className='content-card'; 
  el.innerHTML=`<h2>管理預算</h2><div class="muted">（保留原本預算掛載點）</div>`; 
  $('#pageHost', mount).replaceChildren(el);
}
...
async function listRates(){ 
  const el=document.createElement('div'); 
  el.className='content-card'; 
  el.innerHTML=`<h2>管理貨幣</h2><div class="muted">（保留原本貨幣掛載點）</div>`; 
  $('#pageHost', mount).replaceChildren(el);
}
...
(async function init(){
  ...
  // ✅ 使用 onAuthStateChanged 持續監聽登入狀態
